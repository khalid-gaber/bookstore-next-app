...
async function handleSubmit(e: React.FormEvent, userName: string, email: string, pass: string, dispatch: Dispatch, router: AppRouterInstance) {
    e.preventDefault();
  ...
  return (
    <div className='h-full flex justify-center items-center '>
        <form onSubmit={(e)=>handleSubmit(e, userName, email, pass, dispatch, router)} className='form'>
            ...
            <label>
                <span>email</span>
                <input type="text" value={email} onChange={(e: React.ChangeEvent<HTMLInputElement>)=>setEmail(e.target.value)} className='border' />
            </label>
            <label>
                <span>password</span>
